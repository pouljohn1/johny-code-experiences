...
<AiVideo script={`Welcome back! We’re rendering lists and conditionally showing UI.

You’ll map arrays to components, choose stable keys, and avoid index pitfalls.

Then we’ll compare &&, ternary, and early-return patterns for empty and loading states.

Practice with a small list feature. Scroll down and let’s build!`} id="c7bfc6455e756f2a314b50dbfb8fe5a8c40f81a1872738c4c75ad3db365cc017" />
...
* Missing `key` or using unstable keys → leads to wrong item updates
* Rendering large lists without virtualization → can hurt performance (fine for small lists)
* Over-nesting conditionals → prefer guard clauses and early returns
...
<Woz title="Recap" description="Make sure you really understand 🙃" prompt="Ask me to pick the right key for a dataset, choose a conditional rendering pattern for a scenario, and refactor a nested conditional into early returns." />