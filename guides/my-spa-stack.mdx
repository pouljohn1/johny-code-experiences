...
* **Web** – the main browser UI where users manage their alerts.
* **Mobile** – a cross‑platform app for iOS and Android.
* **Backend** – APIs, business logic, and integrations.
* **Workers** – background jobs for scraping and processing.
* **Database** – persistent storage for all application data.
* **DevOps** – hosting, deployment, and infrastructure tooling.
...
* It is **fast** (package installs, scripts, and dev server start‑up).
* Acts as a **runtime**, test runner, and package manager.
* Has built‑in support for workspaces.

Typical folder structure:

```bash
packages/
├── common/          # Shared utilities, domain logic, helpers
├── common-frontend/ # Shared UI logic, hooks, and components
├── db-schema/       # Prisma schema, migrations, DB models
├── backend/         # API server and background tasks entrypoints
├── jobs/            # One-off or scheduled jobs reusing multiple packages
├── mobile/          # React Native / Expo app
├── web/             # Web SPA built with React + Vite
└── worker/          # Scraping and data processing workers
```
...
* **Affordable** – good performance at a low price point.
* **Flexible** – easy to resize servers as usage grows.
* **Simple networking** – private networks and firewalls are straightforward.
...
* Excellent **DX** for React + Vite/Next‑style frontends.
...
* Great tooling: over‑the‑air updates, dev client, and build services.
...
* **Bun** – the runtime for the backend and workers.
...
* **Prisma** – type‑safe ORM for PostgreSQL.
  * Schema‑driven: the `schema.prisma` file is the single source of truth.
...
* **AWS S3 (or compatible storage)** – file storage.
  * Stores user‑uploaded images and media.
...
* Data enrichment and AI‑powered comparisons.
...
* Map‑based browsing and selection.
...
* **Expo**
  ...
  * Provides batteries‑included APIs (notifications, media, etc.).
* **React Native**
...
* **TypeScript**
  ...
  * Safer cross‑platform refactors.
* **react-native-maps**
...
* Give users on‑the‑go access to their alerts.
...
* **Puppeteer**
  ...
  * Used to load pages, handle JavaScript‑heavy sites, and extract data.
...
7. Files and images are stored in **S3‑compatible** object storage.

The monorepo, shared types, and Bun‑based tooling are what make this multi‑surface setup manageable as a single product.

## Common pitfalls and trade‑offs

Some trade‑offs in this stack:

* **Monorepo complexity** – tooling is great but CI and caching require thought.
* **Bun maturity** – fast and promising, but newer than Node; some packages may need workarounds.
* **Scraping maintenance** – external sites change HTML and structures, breaking scrapers.
* **Vendor mix** – Hetzner, Vercel, Expo, AWS, Stripe, and OpenAI each have their own dashboards and limits.
...
This was a high‑level overview of the technology stack behind domalert.pl. In future guides I plan to:

* Deep‑dive into the **backend architecture** and key patterns.
* Explain the **scraping pipeline** end‑to‑end.
* Show how I structure shared code between **web** and **mobile**.

If you have questions or want more detail about a specific layer, this overview should give you the vocabulary to ask precisely what you’re curious about.

<Woz title="Recap" description="Make sure you really understand 🙃" prompt="Ask me 3 specific questions about this stack: one about hosting, one about the monorepo structure, and one about a particular technology choice and its role." />